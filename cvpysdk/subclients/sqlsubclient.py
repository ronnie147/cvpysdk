...
class SQLServerSubclient(Subclient):
    ...
    def _get_subclient_content_(self, subclient_properties):
        ...
        """
        contents = []
        if 'content' in subclient_properties:
        ...
        if 'mssqlFFGDBName' in subclient_properties['mssqlSubClientProp']:
            database_name = str(subclient_properties['mssqlSubClientProp']['mssqlFFGDBName'])
    ...
    def _initialize_subclient_properties(self):
        ...
        if 'logBackupStoragePolicy' in storage_device:
            self._log_backup_storage_policy = str(storage_device['logBackupStoragePolicy']['storagePolicyName'])
        else:
    ...
    def content(self, value):
        ...
        if self._is_file_group_subclient:
            raise SDKException(
                'Subclient', '102', 'Updating File/File Group Content is not allowed. \
